...
def main(args):
    ...
    for index, filename in enumerate(os.listdir(args.inputs_path)):
        st = time.time() 
        ...
        if name_img != "output":
            ...
            lr_image = cv2.imread(args.inputs_path + "/" + filename, cv2.IMREAD_UNCHANGED).astype(np.float32) / 255.0
            ...
            output_path = f"{args.output_path}/{name_img}.jpg"
            cv2.imwrite(output_path, sr_image)
            
            et = time.time()
            elapsed_time = et - st
            print(f"SR image name {name_img} - {elapsed_time}s")
